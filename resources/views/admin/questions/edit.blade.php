                            ...
                            <form action="{{ route('admin.topic.edit', $topic) }}" method="post">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('subject') is-invalid @enderror" name="subject"
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}" @if (old('subject')) {{ old('subject') == $subject->id ? 'selected' : '' }} @else {{ $subject->id == $topic->subject_id ? 'selected' : '' }} @endif>{{ $subject->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                        name="name" id="name" value="{{ old('name') ?? $topic->name }}"
